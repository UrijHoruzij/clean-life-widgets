 ...
 * @see WP_Widget
 */
abstract class Widget_Media extends WP_Widget {
	...
	public function __construct( $id_base, $name, $widget_options = array(), $control_options = array() ) {
		$widget_opts = wp_parse_args(
			$widget_options,
			array(
				'description'                 => __( 'A media item.','clean-life-widgets' ),
		...
		$l10n_defaults = array(
			'no_media_selected'          => __( 'No media selected','clean-life-widgets' ),
			'add_media'                  => _x( 'Add Media', 'label for button in the media widget','clean-life-widgets' ),
			'replace_media'              => _x( 'Replace Media', 'label for button in the media widget; should preferably not be longer than ~13 characters long','clean-life-widgets' ),
			'edit_media'                 => _x( 'Edit Media', 'label for button in the media widget; should preferably not be longer than ~13 characters long','clean-life-widgets' ),
			'add_to_widget'              => __( 'Add to Widget','clean-life-widgets'),
			'missing_attachment'         => sprintf(
				/* translators: %s: URL to media library */
				__( 'We can&#8217;t find that file. Check your <a href="%s">media library</a> and make sure it wasn&#8217;t deleted.','clean-life-widgets' ),
				esc_url( admin_url( 'upload.php' ) )
			),
			/* translators: %d: widget count */
			'media_library_state_multi'  => _n_noop( 'Media Widget (%d)', 'Media Widget (%d)','clean-life-widgets' ),
			'media_library_state_single' => __( 'Media Widget','clean-life-widgets' ),
			'unsupported_file_type'      => __( 'Looks like this isn&#8217;t the correct kind of file. Please link to an appropriate file instead.','clean-life-widgets' ),
		);
	...
	public function get_instance_schema() {
		$schema = array(
			'attachment_id' => array(
				...
				'default'     => 0,
				'minimum'     => 0,
				'description' => __( 'Attachment post ID','clean-life-widgets' ),
				'media_prop'  => 'id',
			),
			'url'           => array(
				'type'        => 'string',
				'default'     => '',
				'format'      => 'uri',
				'description' => __( 'URL to the media file','clean-life-widgets' ),
			),
			'title'         => array(
				'type'                  => 'string',
				'default'               => '',
				'sanitize_callback'     => 'sanitize_text_field',
				'description'           => __( 'Title for the widget','clean-life-widgets' ),
